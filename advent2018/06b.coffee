...
examine = (x, y) ->
 do (distances = (coords.map (c) -> (dist c, x, y))) ->
  if (distances.reduce (a,b) -> a+b) < 10000
   cnt = cnt + 1

# Check negative points, it doesn't seem impossible that they could be included.
(((examine x, y) for y in [-dim .. dim]) for x in [-dim .. dim])
